...
struct renderer : viewer_type
{
    ...
    using material_type             = material_wrapper;//generic_material<material0, material1, material2, material3>;
...
struct pathtracer_parameters
{
    ...
    using light_type        = point_light<float>;//struct {};
...
#if 1
    for (auto mat : rend.mod.materials)
    {
        // Add emissive material if emissive component > 0
        if (length(mat.ce) > 0.0f)
        {
            ...
            em.ce() = from_rgb(mat.ce);
            em.ls() = 1.0f;
            rend.host_materials.push_back(material_wrapper(new emissive_type(em)));
        }
        else if (mat.specular_exp < 0.001f)
        {
            matte<float> ma;
            ...
            ma.ka() = 1.0f;
            ma.kd() = 1.0f;
            rend.host_materials.push_back(material_wrapper(new matte_type(ma)));
        }
        else if (mat.specular_exp > 100.0f)
        {
            mirror<float> mi;
            ...
            mi.ior() = from_rgb(mat.ior);
            mi.absorption() = from_rgb(mat.absorption);
            rend.host_materials.push_back(material_wrapper(new mirror_type(mi)));
        }
        else
        {
            plastic<float> pl;
            ...
            pl.ks() = 1.0f;
            pl.specular_exp() = mat.specular_exp;
            rend.host_materials.push_back(material_wrapper(new plastic_type(pl)));
        }
    }
#else
