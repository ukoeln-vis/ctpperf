...
//  using primitive_type            = generic_primitive<model::triangle_type, basic_sphere<float>>;
    using primitive_type            = primitive_base*;
    ...
    model mod;
#ifdef INTERSECT_PER_PRIMITIVE_TYPE
    index_bvh<model::triangle_type>             host_bvh1;
    index_bvh<basic_sphere<float>>              host_bvh2;
#else
...
#ifdef __CUDACC__
#ifdef INTERSECT_PER_PRIMITIVE_TYPE
    cuda_index_bvh<model::triangle_type>        device_bvh1;
    cuda_index_bvh<basic_sphere<float>>         device_bvh2;
#else
...
template <typename Primitives1, typename Primitives2, typename Materials, typename Textures>
auto make_parameters(
        Primitives1 const&              begin1,
        Primitives1 const&              end1,
        Primitives2 const&              begin2,
        Primitives2 const&              end2,
        renderer::normal_type const*    normals,
        renderer::tex_coord_type const* tex_coords,
    ...
    -> std::tuple<
        pathtracer_parameters<Primitives1, Materials, Textures>,
        pathtracer_parameters<Primitives2, Materials, Textures>
        >
{
#define ENTRY__(TYPE, BEGIN, END)                                                       \
            pathtracer_parameters<TYPE, Materials, Textures>{                           \
                { BEGIN, END },                                                         \
                normals,                                                                \
                tex_coords,                                                             \
                materials,                                                              \
    ...
    return std::make_tuple(
            ENTRY__(Primitives1, begin1, end1),
            ENTRY__(Primitives2, begin2, end2)
            );
...
#ifdef INTERSECT_PER_PRIMITIVE_TYPE
    std::vector<index_bvh<model::triangle_type>::bvh_ref> bvhs1;
    std::vector<index_bvh<basic_sphere<float>>::bvh_ref>  bvhs2;

    bvhs1.push_back(host_bvh1.ref());
    bvhs2.push_back(host_bvh2.ref());
#else
...
//                            ;
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
    thrust::device_vector<cuda_index_bvh<model::triangle_type>::bvh_ref> device_primitives1;
    thrust::device_vector<cuda_index_bvh<basic_sphere<float>>::bvh_ref> device_primitives2;

    device_primitives1.push_back(device_bvh1.ref());
    device_primitives2.push_back(device_bvh2.ref());
#else
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
            thrust::raw_pointer_cast(device_primitives1.data()),
            thrust::raw_pointer_cast(device_primitives1.data()) + device_primitives1.size(),
            thrust::raw_pointer_cast(device_primitives2.data()),
            thrust::raw_pointer_cast(device_primitives2.data()) + device_primitives2.size(),
#else
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
            bvhs1.data(),
            bvhs1.data() + bvhs1.size(),
            bvhs2.data(),
            bvhs2.data() + bvhs2.size(),
#else
            prims_begin,
            prims_end,
#endif
    ...
    if (frame_num % 100 == 0)
    {
        ...
        for (int y = 0; y < rendertarget.height(); ++y)
        {
            ...
            {
                int index_src = y * rendertarget.width() + x;
                int index_dst = (rendertarget.height() - y - 1) * rendertarget.width() + x;
                colors[index_dst * 3]     = (int)(clamp(powf(tmp[index_src].x, 1/2.2f), 0.f, 1.f) * 255.f);
                colors[index_dst * 3 + 1] = (int)(clamp(powf(tmp[index_src].y, 1/2.2f), 0.f, 1.f) * 255.f);
                colors[index_dst * 3 + 2] = (int)(clamp(powf(tmp[index_src].z, 1/2.2f), 0.f, 1.f) * 255.f);
            }
        }
        pnm_image out_img(
            rendertarget.width(),
        ...
        out_img.save("/local/visionaray/build/" + p.filename().string() + ".ppm", {opt1});
    }
#endif
}
...
int main(int argc, char** argv)
{
    ...
    for (auto p : rend.mod.primitives)
    {
        ...
        primitives.push_back(new triangle_prim(p));
    }

    model::material_type sphere_mat;
    sphere_mat.cs = vec3(1.0f);
    sphere_mat.specular_exp = 1000.f;
    rend.mod.materials.push_back(sphere_mat);

    model::texture_type::ref_type tex(0, 0);
    rend.mod.textures.push_back(tex);

    int sphere_geom_id = rend.mod.materials.size() - 1;

#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
    auto& primitives1 = rend.mod.primitives;
    aligned_vector<basic_sphere<float>> primitives2;
#endif
...
#define R 0.15f
    ...
    {
#ifdef CONFERENCE
        float x = (rand() / (float)RAND_MAX) * 1400;
        float y = (rand() / (float)RAND_MAX) * 400 + 100;
        float z = (rand() / (float)RAND_MAX) * 1200 - 600;
        float r = (rand() / (float)RAND_MAX) * R;
#elif defined(SAN_MIGUEL)
        float x = (rand() / (float)RAND_MAX) * 14;
        float y = (rand() / (float)RAND_MAX) * 4 + 1;
        float z = (rand() / (float)RAND_MAX) * 12 - 6;
        float r = (rand() / (float)RAND_MAX) * R / 100.f;
#endif
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
        sphere.prim_id = i;
        sphere.geom_id = sphere_geom_id | 0x80000000;
        primitives2.push_back(sphere);
#else
        sphere.prim_id = primitives.size();
        sphere.geom_id = sphere_geom_id;
        primitives.push_back(new sphere_prim(sphere));
#endif
    }
...
#ifdef INTERSECT_PER_PRIMITIVE_TYPE
    rend.host_bvh1 = build<index_bvh<model::triangle_type>>(
            primitives1.data(),
            primitives1.size(),
            rend.builder == renderer::Split
            );
    std::cout << "Triangles: " << primitives1.size() << '\n';
    std::cout << "Nodes: " << rend.host_bvh1.nodes().size() << '\n';

    rend.host_bvh2 = build<index_bvh<basic_sphere<float>>>(
            primitives2.data(),
            primitives2.size(),
            rend.builder == renderer::Split
            );

    std::cout << "Spheres: " << primitives2.size() << '\n';
    std::cout << "Nodes: " << rend.host_bvh2.nodes().size() << '\n';
#else
    ...
    std::cout << "Nodes: " << rend.host_bvh.nodes().size() << '\n';
#endif
    ...
    // Copy data to GPU
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
    TRY_ALLOC(rend.device_bvh1 = cuda_index_bvh<model::triangle_type>(rend.host_bvh1));
    TRY_ALLOC(rend.device_bvh2 = cuda_index_bvh<basic_sphere<float>>(rend.host_bvh2));
#else
