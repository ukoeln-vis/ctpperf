...
//#define INTERSECT_PER_PRIMITIVE_TYPE

#if defined(SORT_BY_MATERIAL_TYPE) && defined(INTERSECT_PER_PRIMITIVE_TYPE)
// Would have to handle parameter pack unfolding differently..
#error "These don't go well together.."
#endif
...
public:
    ...
    struct intersect_next_t
    {
        intersect_next_t(pathtracer& parent, size_t compact)
            : parent(parent)
            , compact(compact)
        {
        }
        ...
        {
            parent.primitive_intersect(
                    parent.pointer_cast(parent.rays.data()),
                    parent.pointer_cast(parent.hit_recs.data()),
                    parent.pointer_cast(parent.indices.data()),
                    parent.pointer_cast(parent.indices.data()) + compact,
                    par.prims.begin,
                    par.prims.end
                    );
        }
...
public:
    ...
    void frame(Params params_pack, RT& rt, camera const& cam, unsigned& frame_num)
    {
        ...
        for (unsigned bounce = 0; bounce < params.num_bounces; ++bounce)
        {
            ...
            std::fill(hit_recs.begin(), hit_recs.end(), HR());
#endif
            intersect_next_t intersect_next(*this, compact);
            for_each(params_pack, intersect_next);
#else
        ...
        if (frame_num % 20 == 0)
        {
            ...
            std::cout << "Intersection: " << in << ", Mrays/s: " << (num_rays / 1000000.0) / in << std::endl;
            std::cout << "Sorting     : " << so << ", Mrays/s: " << (num_rays / 1000000.0) / so << std::endl;
            std::cout << "Shading     : " << sh << ", Mrays/s: " << (num_rays / 1000000.0) / sh << std::endl;
            std::cout << "Compaction  : " << co << ", Mrays/s: " << (num_rays / 1000000.0) / co << std::endl;
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
                    update_if(hit_recs[*it], result, result.hit && result.t >= 0.0f && result.t < hit_recs[*it].t);
#else
                    hit_recs[*it] = result;
#endif
#else
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
                    auto hr = closest_hit(rays[*it], prims_first, prims_last);
                    update_if(hit_recs[*it], hr, hr.hit && hr.t < hit_recs[*it].t);
#else
            ...
            )
    {
        ...
        parallel_for(
            ...
            [=] __device__ (int index, random_sampler<S>& samp)
            {
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
                bool is_sphere = hit_rec.geom_id & 0x80000000;
                auto p = std::get<0>(params_pack);
                auto& params = p;
                hit_rec.geom_id = hit_rec.geom_id & ~0x80000000;
#endif
...
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
                using Surface = decltype(get_surface(hit_rec, params));

                Surface surf;
                if (is_sphere)
                    surf = get_surface(hit_rec, std::get<1>(params_pack));
                else
                    surf = get_surface(hit_rec, std::get<0>(params_pack));
#else
                ...
                S pdf(0.0);
#if defined(INTERSECT_PER_PRIMITIVE_TYPE)
                spectrum<S> src;// = surf.sample(sr, refl_dir, pdf, samp);

                if (is_sphere)
                {
                    auto sr     = make_shade_record<decltype(std::get<1>(params_pack)), S>();
                    sr.active   = hit_rec.hit;
                    sr.normal   = n;
                    sr.view_dir = view_dir;

                    src = surf.sample(sr, refl_dir, pdf, samp);
                }
                else
                {
                    auto sr     = make_shade_record<decltype(std::get<0>(params_pack)), S>();
                    sr.active   = hit_rec.hit;
                    sr.normal   = n;
                    sr.view_dir = view_dir;
...
//{
//    static std::mutex m;std::unique_lock<std::mutex> l(m);
//    if (!is_sphere)
//        std::cout << params.ambient_color << std::endl;
//}
                if (emissive)
                {
                    *it = ~(*it);
