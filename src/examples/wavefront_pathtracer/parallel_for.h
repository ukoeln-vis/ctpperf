...
template <typename Func>
void parallel_for(int first, int last, Func func, int stream)
{
    static bool init = false;
    static cudaStream_t streams[8];
    if (!init)
    {
        for (int i = 0; i < 8; ++i)
    ...
    int block_size = 64;
    int grid_size = div_up(n, block_size);
    detail::kernel<<<grid_size, block_size, 0, streams[stream]>>>(func, n);
}
...
template <typename Sampler, typename Func>
void parallel_for(Sampler /* */, int first, int last, Func func, int stream)
{
    static bool init = false;
    static cudaStream_t streams[8];
    if (!init)
    ...
    int block_size = 64;
    int grid_size = div_up(n, block_size);
    detail::kernel<<<grid_size, block_size, 0, streams[stream]>>>(Sampler{}, func, n);
}
